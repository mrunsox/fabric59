...
| 3 — Page-type unification | **Done** | Canonical page-type primitives (`ListPage` · `DetailPage` · `BuilderPage` · `ConfigPage` · `LogPage`) under `src/components/workspace/page-types/`; Library shell at `/w/:id/library` (Guides + Templates + read-only Blueprints; standalone `/guides` and `/templates` stay mounted); sectioned Settings shell (Workspace / Brand / Readiness / Members / Brain / Advanced) with `?section=` deep-links; Notifications reframed as Log + Config tabs; Dispositions effective-source note; Form / Client / Integration detail pages adopt `DetailPage`. |
| 4 — Operator cockpit + live workflow polish | **Done** | Cockpit shell at `/w/:id/cockpit` with `?tab=live\|supervisor\|runs` (embeds the existing Agent / Supervisor / Runs pages; standalone routes remain mounted); `cockpit` nav repointed to `/cockpit`; In-call pill in cockpit top bar; QA queue gains `Open in Runs` deep-link consuming `?search=` on Runs; Analytics drill-downs add Cockpit + Runs. |
