...
import {
  useFive9CallVariables,
  useUpsertCallVariable,
  useDeleteCallVariable,
} from "@/hooks/useFive9Overlay";
...
export default function CampaignVariablesPanel({ clientId, organizationId, campaignId }: Props) {
  ...
  const campaignVars = (vars ?? []).filter((v: any) =>
    campaignId ? v.campaign_id === campaignId || v.campaign_id == null : true,
  );
  ...
  const handleAdd = () => {
    if (!draft.internal_name || !draft.display_label) return;
    upsert.mutate({
      ...draft,
      client_id: clientId,
    ...
    <Card>
      <CardHeader>
        <div className="flex items-center gap-2">
          ...
          <CardTitle className="text-base">Call Variables</CardTitle>
        </div>
        <CardDescription>
          Variables agents capture during this campaign and how they map to the canonical Fabric59
          model. Campaign-scoped — no provider credentials live here.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="grid grid-cols-2 lg:grid-cols-6 gap-3">
          ...
          <div>
            ...
            <Select
              value={draft.fabric59_field_path || "none"}
              onValueChange={(v) =>
                setDraft({ ...draft, fabric59_field_path: v === "none" ? "" : v })
              }
            >
              <SelectTrigger>
        ...
        <Table>
          ...
          <TableBody>
            ...
            {!isLoading && campaignVars.length === 0 && (
              <TableRow>
                <TableCell colSpan={5} className="text-center text-muted-foreground py-8">
                  No call variables yet — add one above
                </TableCell>
              </TableRow>
            )}
            {campaignVars.map((v: any) => (
              <TableRow key={v.id}>
                <TableCell>
