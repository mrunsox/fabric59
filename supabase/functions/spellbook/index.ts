...
serve(async (req) => {
  ...
  try {
    ...
    return new Response(JSON.stringify({ success: true, tenant_id: tenantId, action, message: `Spellbook ${action} endpoint — API integration pending`, payload }), { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
  } catch (error: unknown) {
